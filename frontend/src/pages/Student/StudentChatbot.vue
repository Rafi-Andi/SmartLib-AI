...
const sendMessage = () => {
  ...
  setTimeout(() => {
    ...
    messages.value.push({
      id: Date.now(),
      sender: 'ai',
      text: 'Fitur integrasi bahasa alami sedang dalam pengembangan. Saya akan segera bisa melayani semua pertanyaan Anda secara otomatis!',
      time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
    })
    scrollToBottom()
  }, 1500)
...
<template>
  <div class="flex flex-col h-[calc(100dvh-72px)] bg-slate-50 font-sans relative overflow-hidden">
    <header class="bg-white/80 backdrop-blur-md px-4 py-3 flex items-center shadow-sm border-b border-slate-100 z-10">
      <div class="w-10 h-10 bg-gradient-to-br from-primary-400 to-primary-600 rounded-xl flex items-center justify-center mr-3 shadow-md relative">
      ...
      <button class="ml-auto p-2 text-slate-400 hover:text-slate-600 rounded-full hover:bg-slate-100 transition-colors">
    ...
    >
      ...
      >
        <div v-if="msg.sender === 'ai'" class="w-8 h-8 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 flex-shrink-0 flex items-center justify-center mr-2 shadow-sm mt-auto">
        ...
        >
          ...
          >
            {{ msg.time }}
            <svg v-if="msg.sender === 'user'" class="w-3 h-3 text-primary-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div v-if="isTyping" class="flex justify-start">
        <div class="w-8 h-8 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 flex-shrink-0 flex items-center justify-center mr-2 mt-auto">
          <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
             <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
          </svg>
        </div>
        <div class="bg-white border border-slate-100 rounded-2xl rounded-tl-sm px-4 py-4 flex items-center space-x-1.5 shadow-sm">
    ...
    <div class="p-3 bg-white border-t border-slate-100 shadow-[0_-4px_15px_-3px_rgba(0,0,0,0.05)] relative z-10">
      <div class="flex items-end gap-2 bg-slate-50 p-1.5 rounded-[24px] border border-slate-200 focus-within:border-primary-400 focus-within:ring-2 focus-within:ring-primary-100 transition-all duration-300">
        
        <button class="p-2.5 text-slate-400 hover:text-primary-500 hover:bg-white transition-colors rounded-full flex-shrink-0">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
