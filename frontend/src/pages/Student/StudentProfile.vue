...
<template>
  <div class="min-h-screen flex flex-col pb-20 text-slate-800">
    <header class="bg-primary-600 p-6 pb-16 text-white">
    ...
    <div class="px-4 -mt-12">
      <div class="bg-white border border-slate-200 rounded-2xl p-6">
        <div class="flex items-center gap-4">
          <div
            class="w-20 h-20 bg-primary-600 rounded-full flex items-center justify-center text-3xl font-bold text-white">
            {{ dataProfile?.name[0] }}
          </div>
          <div class="flex-1">
            <h2 class="text-xl font-bold">{{ dataProfile?.name }}</h2>
            <p class="text-slate-600">{{ dataProfile?.email }}</p>
            <div class="flex items-center gap-2 mt-2">
              <span class="px-2 py-1 bg-accent-500/20 text-accent-400 rounded text-xs">Siswa</span>
              <span v-if="dataProfile?.rfid_uid"
    ...
    <div class="p-4 space-y-4">
      <div class="grid grid-cols-2 gap-4">
        <div class="bg-white border border-slate-200 rounded-xl p-4 text-center">
          ...
          <p class="text-xs text-slate-600 mt-1">Total Dipinjam</p>
        </div>
        <div class="bg-white border border-slate-200 rounded-xl p-4 text-center">
          <p class="text-2xl font-bold text-warning-500">{{ dataProfile?.totalBorrowActive }}</p>
          <p class="text-xs text-slate-600 mt-1">Sedang Dipinjam</p>
        </div>
      </div>
      ...
      <div class="bg-white border border-slate-200 rounded-2xl p-4">
        ...
        <div class="space-y-4">
          ...
          <div class="flex items-center justify-between py-2">
            <span class="text-slate-600">Member Sejak</span>
            <span>{{ formatDate(dataProfile?.created_at) }}</span>
          </div>
        </div>
      </div>

      <div v-if="dataProfile?.rfid_uid" class="bg-primary-500/20 border border-primary-500/30 rounded-2xl p-4">
        <div class="flex items-center justify-between">
          ...
          <span class="px-3 py-1 bg-success-500 rounded-full text-sm text-white">Aktif</span>
        </div>
      </div>
      <div v-if="!dataProfile?.rfid_uid" class="bg-red-500/20 border border-red-500/30 rounded-2xl p-4">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
          ...
          <span class="px-3 py-1 bg-red-500 rounded-full text-sm text-white">Tidak Aktif</span>
        </div>
      </div>
